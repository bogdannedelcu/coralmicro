...
def main():
    ...
    for frame in FRAMES:
        ...
        if n > 0:
            frames_ok += 1
    summary = '{"backend":"sentai_sim.markers.whycon","init_rc":%d,'                       '"fx":%.6f,"fy":%.6f,"cx":%.6f,"cy":%.6f,'                       '"marker_diameter_m":%.6f,'                       '"frames":%d,"frames_with_detections":%d,'                       '"detections_total":%d}\n' % (
                  rc,
                  288.3, 288.3, 160.0, 120.0,
                  0.0544,
                  len(FRAMES), frames_ok, total)
