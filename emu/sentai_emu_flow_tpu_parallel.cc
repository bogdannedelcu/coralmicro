// S215/B8 guest-side FlowTask + physical Coral parallel smoke.
//
...
//
// In parallel, a second guest FreeRTOS task starts the production FlowTask and
// feeds SENTAI_PREP_SLOT_FLOW_GRAY_80x60 with varied cat-frame offsets.  There
// is intentionally no PyCoral and no POSIX/SIM runtime in this target.
...
extern "C" volatile uint32_t g_sentai_emu_parallel_done;
...
extern "C" volatile uint32_t g_sentai_emu_flow_base_ready;
extern "C" volatile uint32_t g_sentai_emu_flow_completed;
extern "C" volatile uint32_t g_sentai_emu_flow_detect_ok;
extern "C" volatile uint32_t g_sentai_emu_flow_fail_code;
extern "C" volatile uint32_t g_sentai_emu_flow_elapsed_ms;
extern "C" volatile uint32_t g_sentai_emu_flow_fps_x100;
extern "C" volatile int32_t g_sentai_emu_flow_last_dx_q1000;
extern "C" volatile int32_t g_sentai_emu_flow_last_dy_q1000;
extern "C" volatile uint32_t g_sentai_emu_flow_last_conf;
extern "C" uint8_t g_sentai_emu_flow_base_y80x60[FLOW_GRAY_PIXELS];
...
constexpr uint32_t kBootFailed = 0xEF10;
...
constexpr uint32_t kFailFlowStart = 0xE901;
constexpr uint32_t kFailFlowValidate = 0xE902;
constexpr uint32_t kFailFlowFps = 0xE903;
...
constexpr int kFlowMeasureFrames = 24;
...
StackType_t g_flow_stack[configMINIMAL_STACK_SIZE * 20]
...
uint8_t g_shifted_flow_frame[FLOW_GRAY_PIXELS]
    __attribute__((aligned(32), section(".sdram_bss")));

volatile uint32_t g_sensor_frames = 0;
...
void BuildShiftedFlowFrame(int ox, int oy) {
  for (int y = 0; y < FLOW_GRAY_H; ++y) {
    ...
    for (int x = 0; x < FLOW_GRAY_W; ++x) {
      ...
      g_shifted_flow_frame[y * FLOW_GRAY_W + x] =
          g_sentai_emu_flow_base_y80x60[sy * FLOW_GRAY_W + sx];
    }
  }
}

int PublishFlowFrame(int ox, int oy) {
  BuildShiftedFlowFrame(ox, oy);
  const uint32_t mask = sentai_prep_tick_frame();
  ...
  memcpy(dst, g_shifted_flow_frame, FLOW_GRAY_PIXELS);
...
int WaitFlowProcessed(uint32_t want_seq, int timeout_ms) {
  ...
  const TickType_t total = pdMS_TO_TICKS(static_cast<uint32_t>(timeout_ms));
  while ((xTaskGetTickCount() - start) < total) {
    if (FLOW_SHARED().frames_processed >= want_seq) {
      return 0;
    }
...
int ValidateFlowOffsets() {
  uint32_t result_seq = 0;
  for (uint32_t i = 0; i < static_cast<uint32_t>(sizeof(kOffsets) / sizeof(kOffsets[0])); ++i) {
    const int rc = PublishFlowFrame(kOffsets[i].x, kOffsets[i].y);
    if (rc != 0) return 100 + static_cast<int>(i);
    ++result_seq;
    if (WaitFlowProcessed(result_seq, 5000) != 0) {
      return 200 + static_cast<int>(i);
    }
    SnapshotFlowResult();
    const int exp_dx = (i == 0) ? 0 : static_cast<int>(kOffsets[i].x) - static_cast<int>(kOffsets[i - 1].x);
    const int exp_dy = (i == 0) ? 0 : static_cast<int>(kOffsets[i].y) - static_cast<int>(kOffsets[i - 1].y);
    const int32_t want_dx_q1000 = static_cast<int32_t>(exp_dx * 1000);
    const int32_t want_dy_q1000 = static_cast<int32_t>(exp_dy * 1000);
    int match = 0;
    if (i == 0) {
    ...
    UartWritef("FLOW_VALIDATE frame=%lu exp_dx=%d exp_dy=%d dx_q1000=%ld dy_q1000=%ld conf=%lu match=%d\r\n",
               ...
               exp_dx, exp_dy,
               static_cast<long>(g_sentai_emu_flow_last_dx_q1000),
               static_cast<long>(g_sentai_emu_flow_last_dy_q1000),
               static_cast<unsigned long>(g_sentai_emu_flow_last_conf),
               match);
    if (!match) return 300 + static_cast<int>(i);
  }
...
int MeasureFlowFps() {
  ...
  for (int i = 0; i < kFlowMeasureFrames; ++i) {
    const Offset& off = kOffsets[i % static_cast<int>(sizeof(kOffsets) / sizeof(kOffsets[0]))];
    const int rc = PublishFlowFrame(off.x, off.y);
    if (rc != 0) return 400 + i;
    const uint32_t want = start_processed + static_cast<uint32_t>(i) + 1u;
  ...
  g_sentai_emu_flow_elapsed_ms = static_cast<uint32_t>(elapsed);
  g_sentai_emu_flow_completed = kFlowMeasureFrames;
  g_sentai_emu_flow_fps_x100 =
      static_cast<uint32_t>(static_cast<uint64_t>(kFlowMeasureFrames) * 100000ull /
                            static_cast<uint64_t>(elapsed));
  SnapshotFlowResult();
  UartWritef("FLOW_FPS frames=%lu elapsed_ms=%lu fps_x100=%lu last_dx_q1000=%ld last_dy_q1000=%ld conf=%lu\r\n",
             static_cast<unsigned long>(g_sentai_emu_flow_completed),
             static_cast<unsigned long>(g_sentai_emu_flow_elapsed_ms),
             static_cast<unsigned long>(g_sentai_emu_flow_fps_x100),
             static_cast<long>(g_sentai_emu_flow_last_dx_q1000),
             static_cast<long>(g_sentai_emu_flow_last_dy_q1000),
             static_cast<unsigned long>(g_sentai_emu_flow_last_conf));
  return 0;
...
void FlowBenchTask(void*) {
  UartWrite("FLOW_PARALLEL BEGIN\r\n");
  sentai_prep_init();
  ...
  if (rc != 0) {
    g_sentai_emu_flow_fail_code = kFailFlowStart;
    g_sentai_emu_boot_state = kBootFailed;
    return;
  }
  vTaskDelay(pdMS_TO_TICKS(20));

  rc = ValidateFlowOffsets();
  if (rc != 0) {
    g_sentai_emu_flow_fail_code = kFailFlowValidate + static_cast<uint32_t>(rc);
    g_sentai_emu_boot_state = kBootFailed;
  ...
  rc = MeasureFlowFps();
  if (rc != 0) {
    g_sentai_emu_flow_fail_code = kFailFlowFps + static_cast<uint32_t>(rc);
    g_sentai_emu_boot_state = kBootFailed;
  ...
  UartWritef("FLOW_STATS frames=%lu grab_fail=%lu grab_streak=%lu running=%d\r\n",
             ...
             static_cast<unsigned long>(grab_fail),
             static_cast<unsigned long>(grab_streak),
             running);
  const int stop_rc = sentai_flow_stop();
  UartWritef("FLOW_STOP rc=%d\r\n", stop_rc);
  MaybeMarkParallelPass();
...
void MaybeMarkParallelPass() {
  if (g_sentai_emu_tpu_fail_code == 0 &&
      g_sentai_emu_flow_fail_code == 0 &&
      g_sentai_emu_tpu_completed >= static_cast<uint32_t>(kInvokeRuns) &&
      g_sentai_emu_flow_completed >= static_cast<uint32_t>(kFlowMeasureFrames) &&
      g_sentai_emu_flow_detect_ok == 1) {
    g_sentai_emu_parallel_done = 1;
...
void TpuTask(void*) {
  ...
  UartWrite("TPU_PHYSICAL_SEND_PARALLEL BEGIN\r\n");
...
extern "C" int sentai_detection_is_running(void) {
  return g_sentai_emu_tpu_completed < static_cast<uint32_t>(kInvokeRuns) ? 1 : 0;
}
...
volatile uint32_t g_sentai_emu_parallel_done = 0;
...
extern "C" int main(int argc, char** argv) {
  ...
  UartInit();
  UartWrite("SentAI EMU FlowTask + TPU physical Send* parallel boot\r\n");

  TaskHandle_t tpu_task =
      xTaskCreateStatic(TpuTask, "tpu_phy_send",
                        configMINIMAL_STACK_SIZE * 96, nullptr,
                        tskIDLE_PRIORITY + 2, g_tpu_stack, &g_tpu_tcb);
  TaskHandle_t flow_task =
      xTaskCreateStatic(FlowBenchTask, "flow_parallel",
                        configMINIMAL_STACK_SIZE * 20, nullptr,
                        tskIDLE_PRIORITY + 2, g_flow_stack, &g_flow_tcb);
  if (!tpu_task || !flow_task) {
