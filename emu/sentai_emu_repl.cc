...
void ReplTask(void *) {
    ...
    // Mount an existing emulated user partition if present. Destructive
    // formatting belongs to explicit storage setup/smoke targets; this REPL
    // path should exercise the same FileX/LevelX runtime API shape as board
    // firmware.
    (void)FxUserInit(0);
#endif
    ...
    mp_embed_exec_str(
        ...
        "    print('CALIB_GZ_CALIB_SHARED', len(r), off, is_cal)\n"
        "    sentai.fr.push_scalar('calib_shared_len_R', len(r))\n"
    ...
    mp_embed_exec_str(
        ...
        "    except Exception as e:\n"
        "        check(name, False, type(e).__name__ + ':' + str(e))\n"
        "emit('HW_BEGIN')\n"
        "check('fs_mkdir_fr', sentai.fs.mkdir('/fr') == True)\n"
        "sentai.fr.init()\n"
        "check('fr_open_events', sentai.fr.open('events','/fr/s228_events.csv') == 0)\n"
        "check('fr_open_scalars', sentai.fr.open('scalars','/fr/s228_scalars.csv') == 0)\n"
        ...
        "try:\n"
        "    sentai.usb.read(8, 0)\n"
        "    check('usb_read_requires_open', False, 'read without open')\n"
        "except OSError as e:\n"
        "    check('usb_read_requires_open', 'serial not open' in str(e), str(e))\n"
        "sentai.usb.close()\n"
        "try:\n"
        "    sentai.uart.open(576000)\n"
        "    check('uart_requires_usb_console', False, 'opened on uart console')\n"
        "except OSError as e:\n"
        "    check('uart_requires_usb_console', 'REPL must be on USB' in str(e), str(e))\n"
        "sentai.console('usb')\n"
        "check('uart_open_false', sentai.uart.open(576000) == False)\n"
        "check('uart_available_zero', sentai.uart.available() == 0)\n"
        "try:\n"
        "    sentai.uart.read(8, 0)\n"
        "    check('uart_read_requires_open', False, 'read without open')\n"
        "except OSError as e:\n"
        "    check('uart_read_requires_open', 'UART serial not open' in str(e), str(e))\n"
        "sentai.uart.close()\n"
        "check_unsupported('imu_init_unsupported', lambda: sentai.imu.init())\n"
        "check_unsupported('imu_read_unsupported', lambda: sentai.imu.read())\n"
        "check_unsupported('imu_degrees_unsupported', lambda: sentai.imu.degrees())\n"
        "check_unsupported('imu_tap_start_unsupported', lambda: sentai.imu.tap_start())\n"
        "check_unsupported('imu_poll_unsupported', lambda: sentai.imu.poll_event(0))\n"
        "check_unsupported('mic_start_unsupported', lambda: sentai.mic.start(1))\n"
        "check_unsupported('mic_recording_unsupported', lambda: sentai.mic.recording())\n"
        "check_unsupported('mic_level_unsupported', lambda: sentai.mic.level())\n"
        "check_unsupported('mic_stop_unsupported', lambda: sentai.mic.stop())\n"
        "check_unsupported('mic_save_unsupported', lambda: sentai.mic.save_mp3())\n"
        "check('sleep_namespace_absent', 'sleep' not in dir(sentai), dir(sentai))\n"
        "t0 = sentai.rtos.ticks_ms()\n"
        "sentai.rtos.sleep_ms(1)\n"
        "t1 = sentai.rtos.ticks_ms()\n"
        "check('rtos_sleep_ms_available', t1 >= t0, str(t0) + '->' + str(t1))\n"
        "check('servo_init_sim', sentai.servo.init('sim') == 0)\n"
        "check('servo_arm_sim', sentai.servo.arm() == 0)\n"
        "check('servo_takeoff_sim', sentai.servo.takeoff(0.5) == 0)\n"
        "check('servo_move_sim', sentai.servo.move(0.01, 0, 0) == 0)\n"
        "check('servo_hover_sim', sentai.servo.hover() == 0)\n"
        "check('servo_land_sim', sentai.servo.land() == 0)\n"
        ...
        "check('servo_trace_shared', len(sentai.servo.trace()) >= 6, sentai.servo.trace())\n"
        "check('servo_pose_none', sentai.servo.pose() == None)\n"
        "check('servo_pose_not_ready', sentai.servo.pose_ready() == 0)\n"
        "check('servo_marker_unavailable', sentai.servo.marker_setup_start() < 0)\n"
        "check('servo_marker_stop', sentai.servo.marker_task_stop() == 0)\n"
        "check('calib_default_R', len(sentai.calib.get_R_cam_to_body()) == 9)\n"
        "off = sentai.calib.get_cam_offset_B()\n"
        "check('calib_default_offset', len(off) == 3 and off[0] < 0, off)\n"
        "check('calib_not_calibrated', sentai.calib.is_calibrated() == False)\n"
        "kr = sentai.calib.run_kabsch([])\n"
        "check('calib_kabsch_empty_rejected', len(kr[0]) == 9 and kr[1]['accepted'] == False, kr)\n"
        "check('calib_commit_R_shared', sentai.calib.commit_R((1,0,0,0,1,0,0,0,1)) == 0)\n"
        "check('calib_is_calibrated_after_commit', sentai.calib.is_calibrated() == True)\n"
        "sentai.calib.clear()\n"
        "check('calib_clear_shared', sentai.calib.is_calibrated() == False)\n"
        "check('calib_setup_defaults_unavailable', sentai.calib.setup_defaults() < 0)\n"
        "check('calib_orientation_task_unavailable', sentai.calib.orientation_task_start() < 0)\n"
        "check('calib_orientation_task_stop', sentai.calib.orientation_task_stop() == 0)\n"
        "check('calib_task_start_unavailable', sentai.calib.task_start('x', 1.0, 0.1) < 0)\n"
        "check('calib_task_stop', sentai.calib.task_stop() == 0)\n"
        "check('calib_bringup_unavailable', sentai.calib.run_bringup([(0,0,0)]) < 0)\n"
        ...
        "check('lifter_set_camera_shared', sentai.object_lifter.set_camera(577.0, 579.0, 320.0, 240.0, (0,1,0,1,0,0,0,0,-1), (-0.04,0,-0.02)) == 0)\n"
        "check('lifter_init_shared', sentai.object_lifter.init_from_bbox(7, 4, 320.0, 240.0, 45.0, 0.0625, (0.0,0.0,1.0), 0.0) >= 0)\n"
        "check('lifter_count_one_shared', sentai.object_lifter.count() == 1)\n"
        "check('lifter_world_pos_shared', sentai.object_lifter.world_pos(7) != None)\n"
        "check('lifter_clear_shared', sentai.object_lifter.clear() == 1)\n"
        ...
        "check('safety_enable_markers', sentai.safety.enable_markers(n_min=4, max_loss_s=0.001) == 0)\n"
        "sentai.safety._test_push_aruco(3, 1, 0)\n"
        "check('safety_low_marker_streak_pending', sentai.safety.aborted() == False)\n"
        "sentai.safety._test_push_aruco(3, 2, 2)\n"
        "check('safety_test_abort', sentai.safety.aborted() == True and sentai.safety.reason().startswith('aruco:'))\n"
