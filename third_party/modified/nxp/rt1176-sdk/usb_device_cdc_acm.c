...
usb_status_t USB_DeviceCdcAcmEvent(void *handle, uint32_t event, void *param)
{
    ...
    switch (eventCode)
    {
        ...
        case kUSB_DeviceClassEventSetInterface:
            ...
            {
                /* Comm interface — original handling */
            ...
            {
                /* Check if it's the data interface — needed for CDC-NCM/ECM
                 * where data interface has alt 0 (no endpoints) and alt 1 (bulk endpoints).
                 * We cannot use the shared 'alternate' field because comm and data interfaces
                 * have different alt setting counts. Handle data interface separately. */
                usb_device_interface_list_t *ifList =
                    &cdcAcmHandle->configStruct->classInfomation
                         ->interfaceList[cdcAcmHandle->configuration - 1U];
                uint8_t requestedIface = (uint8_t)(interfaceAlternate >> 8U);
                usb_device_interface_struct_t *newDataIf = NULL;

                for (count = 0; count < ifList->count; count++)
                {
                    if (USB_DEVICE_CONFIG_CDC_DATA_CLASS_CODE ==
                            ifList->interfaces[count].classCode &&
                        ifList->interfaces[count].interfaceNumber == requestedIface)
                    {
                        /* Found data interface — deinit current data endpoints */
                        if (NULL != cdcAcmHandle->dataInterfaceHandle)
                        {
                            uint32_t ep;
                            for (ep = 0;
                                 ep < cdcAcmHandle->dataInterfaceHandle->endpointList.count;
                                 ep++)
                            {
                                (void)USB_DeviceDeinitEndpoint(
                                    cdcAcmHandle->handle,
                                    cdcAcmHandle->dataInterfaceHandle
                                        ->endpointList.endpoint[ep]
                                        .endpointAddress);
                            }
                        }

                        /* Find the requested alt setting */
                        {
                            uint32_t idx;
                            for (idx = 0; idx < ifList->interfaces[count].count; idx++)
                            {
                                if (ifList->interfaces[count]
                                        .interface[idx]
                                        .alternateSetting == alternate)
                                {
                                    newDataIf =
                                        &ifList->interfaces[count].interface[idx];
                                    break;
                                }
                            }
                        }
                        ...
                        cdcAcmHandle->dataInterfaceHandle = newDataIf;

                        /* Init endpoints for new alt setting */
                        {
                            uint32_t ep;
                            for (ep = 0; ep < newDataIf->endpointList.count; ep++)
                            {
                                usb_device_endpoint_init_struct_t epInitStruct;
                                usb_device_endpoint_callback_struct_t epCallback;
                                epInitStruct.zlt = 0;
                                epInitStruct.interval =
                                    newDataIf->endpointList.endpoint[ep].interval;
                                epInitStruct.endpointAddress =
                                    newDataIf->endpointList.endpoint[ep]
                                        .endpointAddress;
                                ...
                                if (USB_IN ==
                                    ((epInitStruct.endpointAddress &
                                     ...
                                     USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT))
                                {
                                    if (USB_ENDPOINT_BULK ==
                                        epInitStruct.transferType)
                                    {
                                        cdcAcmHandle->bulkIn.ep =
                                            epInitStruct.endpointAddress &
                                            USB_DESCRIPTOR_ENDPOINT_ADDRESS_NUMBER_MASK;
                                        cdcAcmHandle->bulkIn.isBusy = 0;
                                        cdcAcmHandle->bulkIn
                                            .pipeDataBuffer =
                                            (uint8_t *)USB_INVALID_TRANSFER_BUFFER;
                                        cdcAcmHandle->bulkIn.pipeStall = 0U;
                                        cdcAcmHandle->bulkIn.pipeDataLen = 0U;
                                        epCallback.callbackFn =
                                ...
                                else
                                {
                                    if (USB_ENDPOINT_BULK ==
                                        epInitStruct.transferType)
                                    {
                                        cdcAcmHandle->bulkOut.ep =
                                            epInitStruct.endpointAddress &
                                            USB_DESCRIPTOR_ENDPOINT_ADDRESS_NUMBER_MASK;
                                        cdcAcmHandle->bulkOut.isBusy = 0;
                                        cdcAcmHandle->bulkOut
                                            .pipeDataBuffer =
                                            (uint8_t *)USB_INVALID_TRANSFER_BUFFER;
                                        cdcAcmHandle->bulkOut.pipeStall = 0U;
                                        cdcAcmHandle->bulkOut
                                ...
                                epCallback.callbackParam = cdcAcmHandle;
                                error = USB_DeviceInitEndpoint(
                                    cdcAcmHandle->handle, &epInitStruct,
                                    &epCallback);
                            }
                        }
                        error = kStatus_USB_Success;
...

        {
            ...
            switch (controlRequest->setup->bRequest)
            {
                ...
                case USB_DEVICE_CDC_REQUEST_SEND_BREAK:
                    ...
                    break;
                default:
                    /* Forward unknown class requests (e.g. CDC-ECM SET_ETHERNET_PACKET_FILTER)
                     * to the application callback instead of STALLing. */
                    reqParam.setupValue = controlRequest->setup->wValue;
                    error = cdcAcmHandle->configStruct->classCallback(
                        (class_handle_t)cdcAcmHandle,
                        (uint32_t)(0x100U + controlRequest->setup->bRequest),
                        &reqParam);
                    break;
            }
        }
