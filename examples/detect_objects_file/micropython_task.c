...
// GC heap size for MicroPython (32 KB should be plenty for simple scripts)
#define MP_GC_HEAP_SIZE (32 * 1024)
...
// Console read/write - implemented in modcoral_hal.cc (C++ bridge)
extern int coral_console_read(char* buf, int size);
extern void coral_console_write(const char* buf, int size);
...
static void repl_puts(const char* s) {
    coral_console_write(s, strlen(s));
}
...
static void repl_replace_line(char* buf, int* pos, const char* new_line) {
    ...
    buf[new_len] = '\0';
    *pos = new_len;
    coral_console_write(buf, new_len);
}
...
static void micropython_repl_task(void* param) {
    ...
    repl_puts("\r\n");
    repl_puts("MicroPython REPL on Coral Dev Board Micro\r\n");
