#pragma once
// sentai_lfs_task.h — Dedicated LFS task interface.
//
...
//
// tcpip_thread never blocks: sentai_lfs_try_serve() is always non-blocking.
...
typedef enum { LFS_REQ_LS = 1, LFS_REQ_RAW = 2 } sentai_lfs_req_type_t;

// Start LFS task (call once from sentai_runtime at boot, before httpd).
void sentai_lfs_task_start(void);
...
//   - READY (path match): returns byte count in sentai_lfs_resp_buf(), caller must
//                         call sentai_lfs_resp_done() when done streaming
...
uint8_t* sentai_lfs_resp_buf(void);

// Signal that lwIP has finished streaming from sentai_lfs_resp_buf().
// Must be called from FsCloseCustom after a successful sentai_lfs_try_serve().
