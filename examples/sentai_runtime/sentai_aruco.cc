...
// OP-S10-W16 ablation 2026-05-19 (operator follow-up): repeat the
// threshold on M7 with the integral image placed in OCRAM (M7's
// on-die SRAM, ~3-cycle access, same physical bank M4 is using).
// This isolates the contribution of "SDRAM-backed integral image
// + 32 KB D-cache absorbing it" vs "OCRAM-backed integral image
// no cache" on the comparison with M4.
//
// 78 KB integral at 161x121 fits the leftover m_ocram after the
// 786 KB TPU staging — no encroachment on production.
// At 160x120 frame size to keep the buffer reasonable.
#define ARUCO_OCRAM_W  160
#define ARUCO_OCRAM_H  120
#ifdef __arm__
static int32_t s_integral_ocram[(ARUCO_OCRAM_W + 1) * (ARUCO_OCRAM_H + 1)]
    __attribute__((section(".ocram_bss"), aligned(32)));
static uint8_t s_gray_ocram[ARUCO_OCRAM_W * ARUCO_OCRAM_H]
    __attribute__((section(".ocram_bss"), aligned(32)));
static uint8_t s_binary_ocram[ARUCO_OCRAM_W * ARUCO_OCRAM_H]
    __attribute__((section(".ocram_bss"), aligned(32)));

static void aruco_thresh_ocram_kernel_(int block) {
    const int W = ARUCO_OCRAM_W, H = ARUCO_OCRAM_H;
    const int stride_i = W + 1;
    for (int x = 0; x <= W; ++x) s_integral_ocram[x] = 0;
    for (int y = 1; y <= H; ++y) {
        int32_t row_sum = 0;
        s_integral_ocram[y * stride_i] = 0;
        for (int x = 1; x <= W; ++x) {
            row_sum += s_gray_ocram[(x-1) + (y-1) * W];
            s_integral_ocram[x + y * stride_i] =
                s_integral_ocram[x + (y-1) * stride_i] + row_sum;
        }
    }
    const int half = block / 2;
    for (int y = 0; y < H; ++y) {
        ...
        for (int x = 0; x < W; ++x) {
            int x0 = x - half;    if (x0 < 0) x0 = 0;
            int x1 = x + half;    if (x1 >= W) x1 = W - 1;
            const int32_t A = s_integral_ocram[(x1+1) + (y1+1) * stride_i];
            const int32_t B = s_integral_ocram[(x0)   + (y1+1) * stride_i];
            const int32_t C = s_integral_ocram[(x1+1) + (y0)   * stride_i];
            const int32_t D = s_integral_ocram[(x0)   + (y0)   * stride_i];
            const int32_t box_sum = A - B - C + D;
            const int32_t box_area = (x1 - x0 + 1) * (y1 - y0 + 1);
            const int32_t mean = box_sum / box_area;
            const uint8_t v = s_gray_ocram[x + y * W];
            s_binary_ocram[x + y * W] = ((int32_t)v < mean - ARUCO_THRESH_C)
                                        ? 1u : 0u;
        }
    }
}

extern "C" uint32_t sentai_aruco_thresh_ocram(int block) {
    if (block < 3 || block > 511) return 0;
    // Build synth frame at 160x120.
    for (int y = 0; y < ARUCO_OCRAM_H; ++y) {
        for (int x = 0; x < ARUCO_OCRAM_W; ++x) {
            uint32_t lfsr = (uint32_t)(y * ARUCO_OCRAM_W + x) * 2654435761u;
            uint8_t noise = (lfsr >> 16) & 0x1F;
            int v = 180 + (x * 40) / ARUCO_OCRAM_W + (int)noise - 8;
            if (v < 0) v = 0;
            if (v > 255) v = 255;
            s_gray_ocram[y * ARUCO_OCRAM_W + x] = (uint8_t)v;
        }
    }
    const int cx = ARUCO_OCRAM_W / 2, cy = ARUCO_OCRAM_H / 2;
    for (int y = cy - 20; y < cy + 20; ++y) {
        for (int x = cx - 20; x < cx + 20; ++x) {
            s_gray_ocram[y * ARUCO_OCRAM_W + x] = 30;
        }
    }
    const uint32_t t0 = aruco_dwt_cyc();
    aruco_thresh_ocram_kernel_(block);
    const uint32_t t1 = aruco_dwt_cyc();
    return t1 - t0;
}
#else
extern "C" uint32_t sentai_aruco_thresh_ocram(int) { return 0; }
#endif
