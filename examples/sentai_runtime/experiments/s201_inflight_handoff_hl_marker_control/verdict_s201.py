...
def load_hl_segment_intervals(journal_path: Path) -> list[dict]:
    segs: list[dict] = []
    current = None
    start_t = None
    target = None
    ...
    for line in lines:
        ...
        try:
            ...
            continue
        if parts[1] == "hl_axis_motion_tick":
            try:
            ...
            if label != current:
                ...
                from_target = None
        elif parts[1] == "hl_axis_motion_segment":
            try:
                data = __import__("ast").literal_eval(parts[2])
            except Exception:
                continue
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("usage: verdict_s201.py ITER_DIR", file=sys.stderr)
        return 2
    workdir = Path(argv[1])
    summary = load_json(workdir / "mission_s201_summary.json")
    gt = load_gt(workdir / "fr_current" / "gt.jsonl")
    phase_intervals = load_phase_intervals(workdir / "mission_s201_journal.txt")

    print("s201 verdict: GT is post-mortem only")
    ...
    print(f"R_loaded_matches_seed={summary.get('R_loaded_matches_seed', False)}")
    ...
    axis_motion = summary.get("hl_axis_motion") or {}
    ...
    print(f"handoff_hl_takeoff_rc={hover.get('hl_takeoff_rc')}")
    print(f"handoff_hl_engage_dur_s={hover.get('hl_engage_dur_s')}")
    print(f"handoff_hl_go_to_rc={hover.get('hl_go_to_rc')}")
    print(f"handoff_hl_hold_dur_s={hover.get('hl_hold_dur_s')}")
    print(f"handoff_hl_lateral_speed_limit_m_s={hover.get('hl_lateral_speed_limit_m_s')}")
    ...
    print(f"hl_axis_motion_ok={axis_motion.get('ok', False)}")
    print(f"hl_axis_motion_abort_reason={axis_motion.get('abort_reason', '')}")
    print(f"hl_axis_motion_step={axis_motion.get('step')}")
    print(f"hl_axis_motion_speed_limit_m_s={axis_motion.get('speed_limit_m_s')}")
    print(f"hl_axis_motion_n_full_min={axis_motion.get('n_full_min', '?')}")
    print(f"hl_axis_motion_n_full_recent_avg={axis_motion.get('n_full_recent_avg', '?')}")
    print(f"hl_axis_motion_extpos_send_ok={axis_motion.get('extpos_send_ok')}")
    print(f"hl_axis_motion_pose_rejects={axis_motion.get('pose_rejects')}")
    print(f"hl_axis_motion_last_est_pose={axis_motion.get('last_est_pose')}")
    for seg in axis_motion.get("segments", []):
        print(
            "hl_axis_segment "
            f"label={seg.get('label')} "
            f"target={seg.get('target')} "
            f"duration_s={seg.get('duration_s')} "
            f"linear={seg.get('linear')} "
            f"go_to_rc={seg.get('go_to_rc')} "
    ...
    mission_ok = summary.get("status") in ("HANDOFF_SMOKE_OK", "HL_AXIS_MOTION_OK")
    ...
    if gt:
        ...
        seg_rows, gt_axis_ok = gt_axis_motion_stats(
            gt, load_hl_segment_intervals(workdir / "mission_s201_journal.txt"))
        for row in seg_rows:
    ...
    ok = mission_ok and (summary.get("status") != "HL_AXIS_MOTION_OK" or gt_axis_ok)
    print(f"success={ok}")
