#!/bin/bash
# s142 — HexPatrol: descriptors per H3 cell during cf2 mission.
#
# Drone visits 2 synthetic targets, computes+stores PHOG+GIST-derived
# 64-byte descriptor per H3 cell.  Verdict: gallery has ≥3 places at
# distinct cells, self-match works, closure < 15 cm.
#
# Usage:   bash run.sh
...
WORKDIR=/tmp/s142_hex_descriptor_patrol
mkdir -p "$WORKDIR"
...
respawn_cf2_at_origin() {
    if [ "${S142_NO_RESPAWN:-0}" = "1" ]; then
        echo "[s142] S142_NO_RESPAWN=1 — skipping respawn (NOT reproducible!)"
        return 0
    fi
    if is_cf2_up; then
        echo "[s142] tearing down running SITL so cf2 respawns at origin"
...
ensure_sitl_up() {
    if is_cf2_up; then
        echo "[s142] cf2 SITL up on UDP 19850"
        return 0
    fi
    echo "[s142] launching SITL stack via $LAUNCH_HYBRID"
    ...
    until is_cf2_up; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s142] FAIL — SITL never came up; see $WORKDIR/sitl.log"
            return 1
        fi
        sleep 2
    done
    echo "[s142] cf2 UDP 19850 ready"
}
...
ensure_bridge_after_sim() {
    ...
    until [ -S /tmp/sentai_cam.sock ]; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s142] FAIL — sentai_sim never opened cam.sock"
            return 1
        fi
        sleep 0.3
    done
    if bridge_alive; then
        echo "[s142] gz_to_uds_bridge already up + accepting"
        return 0
    fi
    if [ -S /tmp/sentai_flow_out.sock ]; then
        echo "[s142] stale flow_out.sock (bridge dead); removing"
        rm -f /tmp/sentai_flow_out.sock
    fi
    echo "[s142] launching gz_to_uds_bridge inside distrobox"
    ...
    until [ -S /tmp/sentai_flow_out.sock ]; do
        if [ $(date +%s) -ge $d2 ]; then
            echo "[s142] FAIL — bridge did not open flow_out.sock"
            return 1
        fi
        sleep 0.3
    done
    echo "[s142] flow_out.sock ready"
}

run_mission() {
    echo "[s142] running mission_explore.py (origin-locked closure)"
...
if [ "$mission_rc" -ne 0 ]; then
    echo "[s142] mission_explore.py rc=$mission_rc — see $WORKDIR/mission_host.log + repl.transcript"
fi
verdict && rc=0 || rc=$?
echo "[s142] exit code $rc"
exit $rc
