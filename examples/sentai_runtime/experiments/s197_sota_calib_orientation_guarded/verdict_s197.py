...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("usage: verdict_s197.py ITER_DIR", file=sys.stderr)
        return 2

    workdir = Path(argv[1])
    summary = _load_json(workdir / "mission_s197_summary.json")
    gt = _load_gt(workdir / "fr_current" / "gt.jsonl")

    print("s197 verdict: post-mortem only; GT is not used by sentai_sim")
