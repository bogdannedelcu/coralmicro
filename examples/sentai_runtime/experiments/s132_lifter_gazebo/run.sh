#!/bin/bash
# s132 — L5 lifter Gazebo integration runner.
#
# Mirrors s130/run.sh: ensures SITL up, ensures gz_to_uds_bridge up,
# runs mission_lifter.py (which spawns sentai_sim), then verdict.py.
# Always force-respawns cf2 at origin per [[experiments-start-from-origin]].
#
# Usage:   bash run.sh
...
WORKDIR=/tmp/s132_lifter_gazebo
mkdir -p "$WORKDIR"

VENV_PY="$REPO_ROOT/venv/bin/python3"
LAUNCH_HYBRID="$REPO_ROOT/sim/scripts/launch_hybrid_cf2.sh"
MISSION="$SCRIPT_DIR/mission_lifter.py"
VERDICT="$SCRIPT_DIR/verdict.py"
STOP_SH="$REPO_ROOT/examples/sentai_runtime/experiments/s127_flowbaseline/stop.sh"

WORLD=sentai_crazysim    # same world as s127/s128/s129/s130

is_cf2_up() { ss -lun 2>/dev/null | grep -q ":19850"; }

respawn_cf2_at_origin() {
    if [ "${S132_NO_RESPAWN:-0}" = "1" ]; then
        echo "[s132] S132_NO_RESPAWN=1 — skipping respawn (NOT reproducible!)"
        return 0
    fi
    if is_cf2_up; then
        echo "[s132] tearing down running SITL so cf2 respawns at origin"
...
ensure_sitl_up() {
    if is_cf2_up; then
        echo "[s132] cf2 SITL up on UDP 19850"
        return 0
    fi
    echo "[s132] launching SITL stack via $LAUNCH_HYBRID"
    ...
    until is_cf2_up; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s132] FAIL — SITL never came up; see $WORKDIR/sitl.log"
            return 1
        fi
        sleep 2
    done
    echo "[s132] cf2 UDP 19850 ready"
}
...
ensure_bridge_after_sim() {
    ...
    until [ -S /tmp/sentai_cam.sock ]; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s132] FAIL — sentai_sim never opened cam.sock"
            return 1
        fi
        sleep 0.3
    done
    if bridge_alive; then
        echo "[s132] gz_to_uds_bridge already up + accepting"
        return 0
    fi
    if [ -S /tmp/sentai_flow_out.sock ]; then
        echo "[s132] stale flow_out.sock (bridge dead); removing"
        rm -f /tmp/sentai_flow_out.sock
    fi
    echo "[s132] launching gz_to_uds_bridge inside distrobox"
    ...
    until [ -S /tmp/sentai_flow_out.sock ]; do
        if [ $(date +%s) -ge $d2 ]; then
            echo "[s132] FAIL — bridge did not open flow_out.sock"
            return 1
        fi
        sleep 0.3
    done
    echo "[s132] flow_out.sock ready"
}

run_mission() {
    echo "[s132] running mission_lifter.py (spawns sentai_sim + lateral pass)"
...
if [ "$mission_rc" -ne 0 ]; then
    echo "[s132] mission_lifter.py rc=$mission_rc — see $WORKDIR/mission_host.log + repl.transcript"
fi
verdict && rc=0 || rc=$?
echo "[s132] exit code $rc"
exit $rc
