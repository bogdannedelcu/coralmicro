...
OUT=/tmp/sentai_s109_$STAMP
mkdir -p "$OUT"
echo "[s109] artifacts → $OUT"
...
echo "[s109] launching PX4 (airframe 4043 = GPS+VPE fused, wind 0.2 m/s)"
...
for i in $(seq 1 60); do
    ...
    if grep -qE "Ready for takeoff|Startup script returned successfully" $OUT/px4.log 2>/dev/null; then
        echo "[s109] PX4 boot OK (${i}s)"; break
    fi
    [ $i -eq 60 ] && { echo "[s109] PX4 TIMEOUT"; tail $OUT/px4.log; exit 2; }
done
...
ARUCO_PID=$!
disown
echo "[s109] aruco→vpe bridge pid=$ARUCO_PID"
sleep 2
...
echo "[s109] launching gz→UDS camera bridge"
...
echo "[s109] waiting 10s for GPS stabilization..."
sleep 10

# 6. EKF convergence (let VPE flow for 5s).
echo "[s109] 5s EKF VPE convergence..."
sleep 5
...
echo "[s109] running OFFBOARD mission..."
...
target=$TARGET_Z
print(f"=== s109 (ArUco VPE + wind 0.2 m/s + stress factors) ===")
