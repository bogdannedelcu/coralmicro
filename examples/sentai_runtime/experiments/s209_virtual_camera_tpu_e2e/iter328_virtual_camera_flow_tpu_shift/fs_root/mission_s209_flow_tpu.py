...
def run(n_frames=N_FRAMES):
    global _event_seen, _callback_events
    _event_seen = 0
    _callback_events = []
    ...
    initial_pipe_count = prev_pipe_count
    ...
    try:
        for i in range(n_frames):
            rec, ev, prev_pipe_seq, prev_pipe_count, prev_flow_seq = _record_frame(
                i, 0 if play_rc > 0 else play_rc,
                prev_pipe_seq, prev_pipe_count, prev_flow_seq)
            records.append(rec)
            events.append(ev)
    finally:
    ...
    result = {
        ...
        "initial_count": initial_pipe_count,
        "final_count": int(sentai.pipeline.frame_count()),
        "expected_events": n_frames,
