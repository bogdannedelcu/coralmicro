...
def _wait_pipeline_after(prev_seq, timeout_ms=PIPE_TIMEOUT_MS):
    pipe = sentai.pipeline.get_ex(timeout_ms)
    if pipe and int(pipe[3]) != int(prev_seq):
        return pipe
    print("PIPE_TIMEOUT_AFTER", prev_seq, "LAST", pipe)
    return None
...
def run(n_frames=N_FRAMES):
    ...
    play_rc = sentai.camera.play(FRAME_DIR, PLAY_FPS, n_frames)
