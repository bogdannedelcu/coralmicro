#!/bin/bash
# s135 — L6 sentai.explore CLEAN origin-locked closure test.
#
# Replaces s134 (mis-calibrated verdict).  Per [[sim-test-must-return-home]]:
# drone MUST return to physical takeoff origin or test is FAILED.
# Force-respawns cf2 at origin and never displaces it before
# explore.start() — L6 home equals world origin.
#
# Usage:   bash run.sh
...
WORKDIR=/tmp/s135_explore_clean
mkdir -p "$WORKDIR"
...
respawn_cf2_at_origin() {
    if [ "${S135_NO_RESPAWN:-0}" = "1" ]; then
        echo "[s135] S135_NO_RESPAWN=1 — skipping respawn (NOT reproducible!)"
        return 0
    fi
    if is_cf2_up; then
        echo "[s135] tearing down running SITL so cf2 respawns at origin"
...
ensure_sitl_up() {
    if is_cf2_up; then
        echo "[s135] cf2 SITL up on UDP 19850"
        return 0
    fi
    echo "[s135] launching SITL stack via $LAUNCH_HYBRID"
    ...
    until is_cf2_up; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s135] FAIL — SITL never came up; see $WORKDIR/sitl.log"
            return 1
        fi
        sleep 2
    done
    echo "[s135] cf2 UDP 19850 ready"
}
...
ensure_bridge_after_sim() {
    ...
    until [ -S /tmp/sentai_cam.sock ]; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s135] FAIL — sentai_sim never opened cam.sock"
            return 1
        fi
        sleep 0.3
    done
    if bridge_alive; then
        echo "[s135] gz_to_uds_bridge already up + accepting"
        return 0
    fi
    if [ -S /tmp/sentai_flow_out.sock ]; then
        echo "[s135] stale flow_out.sock (bridge dead); removing"
        rm -f /tmp/sentai_flow_out.sock
    fi
    echo "[s135] launching gz_to_uds_bridge inside distrobox"
    ...
    until [ -S /tmp/sentai_flow_out.sock ]; do
        if [ $(date +%s) -ge $d2 ]; then
            echo "[s135] FAIL — bridge did not open flow_out.sock"
            return 1
        fi
        sleep 0.3
    done
    echo "[s135] flow_out.sock ready"
}

run_mission() {
    echo "[s135] running mission_explore.py (origin-locked closure)"
...
if [ "$mission_rc" -ne 0 ]; then
    echo "[s135] mission_explore.py rc=$mission_rc — see $WORKDIR/mission_host.log + repl.transcript"
fi
verdict && rc=0 || rc=$?
echo "[s135] exit code $rc"
exit $rc
