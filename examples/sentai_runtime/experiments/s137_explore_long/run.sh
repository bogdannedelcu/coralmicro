#!/bin/bash
# s135 — L6 sentai.explore LONG-DISTANCE exploration (behavior-gated).
#
...
respawn_cf2_at_origin() {
    if [ "${S137_NO_RESPAWN:-0}" = "1" ]; then
        echo "[s137] S137_NO_RESPAWN=1 — skipping respawn (NOT reproducible!)"
        return 0
    fi
    if is_cf2_up; then
        echo "[s137] tearing down running SITL so cf2 respawns at origin"
...
ensure_sitl_up() {
    if is_cf2_up; then
        echo "[s137] cf2 SITL up on UDP 19850"
        return 0
    fi
    echo "[s137] launching SITL stack via $LAUNCH_HYBRID"
    ...
    until is_cf2_up; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s137] FAIL — SITL never came up; see $WORKDIR/sitl.log"
            return 1
        fi
        sleep 2
    done
    echo "[s137] cf2 UDP 19850 ready"
}
...
ensure_bridge_after_sim() {
    ...
    until [ -S /tmp/sentai_cam.sock ]; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s137] FAIL — sentai_sim never opened cam.sock"
            return 1
        fi
        sleep 0.3
    done
    if bridge_alive; then
        echo "[s137] gz_to_uds_bridge already up + accepting"
        return 0
    fi
    if [ -S /tmp/sentai_flow_out.sock ]; then
        echo "[s137] stale flow_out.sock (bridge dead); removing"
        rm -f /tmp/sentai_flow_out.sock
    fi
    echo "[s137] launching gz_to_uds_bridge inside distrobox"
    ...
    until [ -S /tmp/sentai_flow_out.sock ]; do
        if [ $(date +%s) -ge $d2 ]; then
            echo "[s137] FAIL — bridge did not open flow_out.sock"
            return 1
        fi
        sleep 0.3
    done
    echo "[s137] flow_out.sock ready"
}

run_mission() {
    echo "[s137] running mission_explore.py (origin-locked closure)"
...
if [ "$mission_rc" -ne 0 ]; then
    echo "[s137] mission_explore.py rc=$mission_rc — see $WORKDIR/mission_host.log + repl.transcript"
fi
verdict && rc=0 || rc=$?
echo "[s137] exit code $rc"
exit $rc
