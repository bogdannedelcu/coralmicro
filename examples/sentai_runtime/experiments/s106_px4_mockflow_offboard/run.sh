...
SPAWN_POSE='0,0,0.3,0,0,0'   # close to ground; velocity-mode climb from here
HOVER_S=15

STAMP=$(date +%Y%m%d_%H%M%S)
OUT=/tmp/sentai_s106_$STAMP
mkdir -p "$OUT"
echo "[s106] artifacts → $OUT"
...
echo "[s106] launching PX4 (airframe 4041 = flow-only, no GPS)"
...
for i in $(seq 1 60); do
    ...
    if grep -qE "Ready for takeoff|Startup script returned successfully" $OUT/px4.log 2>/dev/null; then
        echo "[s106] PX4 boot OK (${i}s)"; break
    fi
    [ $i -eq 60 ] && { echo "[s106] PX4 TIMEOUT"; tail -30 $OUT/px4.log; exit 2; }
done
...
echo "[s106] launching mock-flow bridge"
distrobox enter $DISTROBOX_NAME -- $OUT/_launch_flow.sh
sleep 3

# 4. EKF convergence (let flow flow for 5s).
echo "[s106] 5s EKF convergence window..."
sleep 5

# 5. OFFBOARD velocity hover.
cat >$OUT/_launch_mission.sh <<EOF
#!/bin/bash
python3 $COR/examples/sentai_runtime/experiments/s106_px4_mockflow_offboard/offboard_velocity_hover.py \
    --takeoff-vz -0.5 --takeoff-s 4 \
    --hover-s $HOVER_S \
    --land-vz +0.3 --land-s 8 \
    > $OUT/mission.log 2>&1
EOF
chmod +x $OUT/_launch_mission.sh
echo "[s106] running OFFBOARD velocity mission..."
