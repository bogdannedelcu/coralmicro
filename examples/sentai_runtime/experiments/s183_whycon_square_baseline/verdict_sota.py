#!/usr/bin/env python3
"""s183 verdict (SOTA-aligned) — multi-marker joint pose recovery via
Kabsch SVD.
...
# Marker world positions — operator-specified SQUARE layout (s183),
# proportions 1:1:0.75 with unit = 0.16 m.  4 corners at (±0.16,
# ±0.16) + 2 mid-bar markers at (±0.12, 0) = ±0.75 × corner_x.
# Total span 32 × 32 cm.  X- and Y-mirror symmetries are present
# (operator preference); resolution comes from (a) Z-reflection
# fix in kabsch_with_assignment + (b) cf2 yaw being known-near-zero
# in hover.
MARKER_WORLD = {
    "NW": np.array([-0.16, +0.16, 0.005], dtype=np.float64),
    "NE": np.array([+0.16, +0.16, 0.005], dtype=np.float64),
    "W":  np.array([-0.12,  0.00, 0.005], dtype=np.float64),
    "E":  np.array([+0.12,  0.00, 0.005], dtype=np.float64),
    "SW": np.array([-0.16, -0.16, 0.005], dtype=np.float64),
    "SE": np.array([+0.16, -0.16, 0.005], dtype=np.float64),
}
MARKER_ORDER = ["NW", "NE", "W", "E", "SW", "SE"]

# Camera intrinsics — must match mission_s183.py.  Derived from cf2
...
def main():
    ...
    print(f"[s183-sota] {len(ticks)} ticks, {len(gt_rows)} GT rows")
    ...
    print(f"[s183-sota] Kabsch fits: {n_kabsch_3plus} (4-marker: {n_kabsch_4}; "
          f"<3 dets: {n_no_assoc})")
    print(f"[s183-sota] paired with GT: {len(rows)}")

    csv_path = out_dir / f"s183_kabsch_{args.tag}.csv"
    ...
    print(f"[s183-sota] wrote {csv_path}")
    if not rows:
        return
    ...
    for ax, (name, gk, ek, color) in zip(axes, meta):
        ...
        ax.text(0.98, 0.02, f"MAE = {mae*100:.2f} cm  (n={len(rows)})",
                 ...
                 bbox=dict(boxstyle="round,pad=0.3", fc="white",
                            ec="gray", alpha=0.85))
    fig.suptitle(
        "s183 — WhyCon + Kabsch SOTA (Arun et al. 1987): drone X/Y/Z "
        "est vs GT  [Krajník-cross scene, cf2 SITL]",
        fontsize=11)
    fig.tight_layout(rect=(0, 0, 1, 0.93))
    out_xyz = out_dir / f"s183_kabsch_xyz_{args.tag}.png"
    fig.savefig(out_xyz, dpi=150)
    print(f"[s183-sota] wrote {out_xyz}")
    ...
    ax2.set_title("s183 — Z time-series: GT vs cf2 EKF vs Kabsch")
    ax2.legend(loc="best", fontsize=9)
    ax2.grid(True, alpha=0.3)
    fig2.tight_layout()
    out_z = out_dir / f"s183_kabsch_z_{args.tag}.png"
    fig2.savefig(out_z, dpi=150)
    print(f"[s183-sota] wrote {out_z}")
