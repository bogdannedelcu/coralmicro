# mission_s183 — WhyCon evaluation in Gazebo SIM (anti-cheat compliant).
#
...
JOURNAL_NAME = "mission_s183_journal.txt"
SUMMARY_NAME = "mission_s183_summary.json"
...
FR_DIR           = "/tmp/s183_whycon_square/fr_current"
...
#   5. hover-and-log
Z_HOLD          = 0.78   # iter-2 (operator 2026-05-20): +30% vs 0.60.
                          # At 0.6 m the square 32×32 cm pad's Y extent
                          # is 0.32 / 0.495 = 65% of vertical FOV; some
                          # markers clip → n=3 only.  0.78 m → 0.32 /
                          # 0.644 = 50% Y, comfortable margin, all 6
                          # in frame.  Operator: "markerii sunt prea
                          # mari in poza".
TAKEOFF_DUR     = 2.5
LAND_DUR        = 2.5
...
# VPE control.  iter-9 fixed the C-side ANNULUS_FACTOR to 1.166 so
# tz is now an honest depth observation — VPE can run continuously
# without the iter-6 positive-feedback loop.
#
# iter-2 (s183) tuning, learned from the iter-1 EKF-blowup crash at
# tick 60: after 60 ticks with n=3 (no VPE → cf2 EKF drifted on IMU
# integration → covariance grew), a SINGLE VPE update at tick 60
# kicked cf2 into instability (z jumped from 0.6 to -0.5 in 2 ticks,
# yaw flipped to ±25 then ±135 deg).  Two fixes:
#   - VPE_MIN_DETS=3 (was 4) so VPE fires from the FIRST tick that
#     has any sensible detections; cf2 EKF stays observable from
#     hover-start, never accumulates the large IMU drift.
#   - Optional VPE rate-limit slew per tick (saturated z delta) to
#     bound any single update's authority.  iter-2 keeps it simple
#     (no rate limit yet) and relies on continuous updates.
ENABLE_VPE      = False  # iter-3: drone climbs cleanly to commanded
                          # altitude without VPE; markers clip when VPE
                          # drives it slightly past target (camera-mount
                          # forward offset = 4 cm shifts the effective
                          # FOV center).  We accept cf2 EKF Z drift in
                          # exchange for stable detection rate.
VPE_TZ_BAND     = 0.30   # unused when ENABLE_VPE=False
VPE_MIN_DETS    = 3      # unused when ENABLE_VPE=False
...
def run():
    ...
    try:
        ...
        #      hook.  Used by s183_replay.py post-mortem to compare
