...
WORKDIR=/tmp/s182_whycon_gazebo
mkdir -p "$WORKDIR"
...
# ---- 1. launch SITL stack --------------------------------------------
echo "[s182] launching SITL stack (world=$WORLD)"
...
until is_cf2_up; do
    if [ $(date +%s) -ge $deadline ]; then
        echo "[s182] FAIL — SITL never came up; see $WORKDIR/sitl.log"
        exit 1
    fi
    sleep 2
done
...
echo "[s182] starting gt_recorder for crazyflie_0 → $GT_OUT"
...
echo "[s182] stopping gt_recorder + bridge"
...
JOURNAL="$FS_ROOT/mission_s182_journal.txt"
SUMMARY="$FS_ROOT/mission_s182_summary.json"
