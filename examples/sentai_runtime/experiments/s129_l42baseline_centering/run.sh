#!/bin/bash
# s129 L4.2Baseline runner — visual servoing centring test.
#
# Mirrors s128/run.sh with one difference: workdir is /tmp/s129_l42baseline.
...
respawn_cf2_at_origin() {
    if [ "${S129_NO_RESPAWN:-0}" = "1" ]; then
        echo "[s129] S129_NO_RESPAWN=1 — skipping respawn (NOT reproducible!)"
        return 0
    fi
    if is_cf2_up; then
        echo "[s129] tearing down running SITL so cf2 respawns at origin"
...
ensure_sitl_up() {
    if is_cf2_up; then
        echo "[s129] cf2 SITL up on UDP 19850"
        return 0
    fi
    echo "[s129] launching SITL stack via $LAUNCH_HYBRID"
    ...
    until is_cf2_up; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s129] FAIL — SITL never came up; see $WORKDIR/sitl.log"
            return 1
        fi
        sleep 2
    done
    echo "[s129] cf2 UDP 19850 ready"
}
...
ensure_bridge_after_sim() {
    ...
    until [ -S /tmp/sentai_cam.sock ]; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s129] FAIL — sentai_sim never opened cam.sock"
            return 1
        fi
        sleep 0.3
    done
    if [ ! -S /tmp/sentai_flow_out.sock ]; then
        echo "[s129] launching gz_to_uds_bridge inside distrobox"
        ...
        until [ -S /tmp/sentai_flow_out.sock ]; do
            if [ $(date +%s) -ge $d2 ]; then
                echo "[s129] FAIL — bridge did not open flow_out.sock"
                return 1
            fi
            sleep 0.3
        done
    fi
    echo "[s129] flow_out.sock ready"
}

run_mission() {
    echo "[s129] running mission_l42.py (will spawn sentai_sim + IBVS)"
...
if [ "$mission_rc" -ne 0 ]; then
    echo "[s129] mission_l42.py rc=$mission_rc — see $WORKDIR/mission.log + repl.transcript"
fi
verdict && rc=0 || rc=$?
echo "[s129] exit code $rc"
exit $rc
