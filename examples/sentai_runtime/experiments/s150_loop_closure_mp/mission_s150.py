# mission_s150.py — MP-only port of s143 loop closure.
#
# Two-lap mission: lap-1 stores 3 places (synthetic descriptors), lap-2
# revisits each and queries the gallery — expects MATCH id=stored_pid
# with high score.  Demonstrates "drone CONSUMES memory" pattern.
#
# Same trajectory both laps so descriptors should match deterministically
# (synth seed = wp_idx).
#
# Reuses crtp_log.py + hex_helpers.py.
...
TAKEOFF_DUR     = 2.0
LAND_DUR        = 2.0
# Slow trajectories — peak velocity ≈ 0.10 m/s for flow tracking.
WAYPOINT_DUR    = 6.0
SAMPLE_DWELL_MS = 600
POSE_PERIOD_MS  = 100
...
def _lap(label, store):
    ...
    for i, wp in enumerate(WAYPOINTS):
        ...
        pose, d = _converge_to(wp[0], wp[1], "%s_wp%d" % (label, i))
        if pose is None:
        ...
        if store:
            ...
            # Build the same synthetic descriptor + cell, query gallery.
...
def run():
    ...
    summary = {
        "name": "mission_s150", "status": "STARTED",
    ...
    try:
        ...
        # Count successful matches (same wp idx → match.id == stored_pid).
        ok = 0
