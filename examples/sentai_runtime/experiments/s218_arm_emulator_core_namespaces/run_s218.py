#!/usr/bin/env python3
"""Run S218: verify core sentai.* namespaces in ARM emulation."""
...
TARGET = "sentai_emu_core_namespace_smoke"
RENODE_SCRIPT = ROOT / "emu/renode/sentai_emu_core_namespace_smoke.resc"
RENODE_UI_SCRIPT = ROOT / "emu/renode/sentai_emu_core_namespace_smoke_ui.resc"
UART_LOG = ROOT / "emu/output/sentai_emu_core_namespace_smoke.log"
...
def next_iter_dir() -> pathlib.Path:
    ...
    out = EXP / f"iter{next_id:02d}_core_namespaces"
...
def parse_core(text: str) -> dict[str, object]:
    ...
    for raw_line in text.splitlines():
        ...
        elif line.startswith("CORE_FAILS|"):
            try:
...
def main() -> int:
    ...
    for name in ("configure", "build"):
        ...
        if proc.returncode != 0:
            (iter_dir / "verdict_s218.json").write_text(
    ...
    symbols = {
        "boot_state": parse_hex("sentai_emu_core_namespace_smoke boot_state",
                                renode_proc.stdout),
        "heartbeat": parse_hex("sentai_emu_core_namespace_smoke heartbeat",
                               renode_proc.stdout),
        "repl_lines": parse_hex("sentai_emu_core_namespace_smoke repl_lines",
                                renode_proc.stdout),
        "last_tick": parse_hex("sentai_emu_core_namespace_smoke last_tick",
                               renode_proc.stdout),
    }
    core = parse_core(uart_text)
    results["symbols"] = symbols
    results["core"] = core
    results["pass"] = (
        renode_proc.returncode == 0
        and symbols.get("boot_state") == 0x0500
        and core.get("saw_begin") is True
        and core.get("saw_end") is True
        and core.get("fails") == 0
        and isinstance(core.get("fs_write_size"), int)
        and core["fs_write_size"] > 0
    )

    (iter_dir / "core_s218.txt").write_text(
        "\n".join(
            line.strip() for line in uart_text.splitlines()
            if line.startswith(("CORE_", "CHECK|", "FS_WRITE|"))
        ) + "\n",
        encoding="utf-8")
    (iter_dir / "verdict_s218.json").write_text(
        json.dumps(results, indent=2) + "\n", encoding="utf-8")
    ...
    print(f"S218 iter: {iter_dir.relative_to(ROOT)}")
    print(f"pass={results['pass']} checks={len(core.get('checks', {}))} "
          f"fails={core.get('fails')}")
