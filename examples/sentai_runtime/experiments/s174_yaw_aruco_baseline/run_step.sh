...
#   3. triggers `import mission_yaw_step; mission_yaw_step.run()`
...
MISSION_FILE="$SCRIPT_DIR/mission_yaw_step.py"
...
stage_mission() {
    ...
    rm -f "$FS_ROOT/mission_yaw_step_journal.txt" \
          "$FS_ROOT/mission_yaw_step_summary.json"
}
...
run_mission() {
    ...
    echo "[s174] running mission_yaw_step (timeout ${MISSION_TIMEOUT_S}s)"
    echo "import mission_yaw_step; r = mission_yaw_step.run(); print('FINAL_STATUS:', r['status'], 'steps:', r['steps_completed'], '/', r['n_steps'], 'last_deg:', r['last_target_deg'], 'is_done:', r['is_done'], 'aborted:', r['aborted_by_safety'])" \
