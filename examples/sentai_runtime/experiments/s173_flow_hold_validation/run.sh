...
#   3. triggers `import mission_flow_hold; mission_flow_hold.run()`
...
MISSION_FILE="$SCRIPT_DIR/mission_flow_hold.py"
...
MISSION_TIMEOUT_S=70          # autotune dur ~30s + takeoff+settle+land ~10s + slack

is_cf2_up() { ss -lun 2>/dev/null | grep -q ":19850"; }

respawn_from_origin() {
    echo "[s173] unconditional teardown"
...
ensure_sitl_up() {
    echo "[s173] launching SITL stack"
    distrobox enter crazysim-garden -- bash "$LAUNCH_HYBRID" "$WORLD" > "$WORKDIR/sitl.log" 2>&1 &
    disown $! 2>/dev/null || true
    local deadline=$(( $(date +%s) + 150 ))
    until is_cf2_up; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s173] FAIL — SITL never came up"; return 1
        fi
        sleep 2
    done
    echo "[s173] cf2 UDP 19850 ready"
}

ensure_bridge_up() {
    echo "[s173] launching gz_to_uds_bridge"
...
start_gt_recorder() {
    ...
    echo "[s173] starting gt_recorder"
...
stage_mission() {
    echo "[s173] staging $MISSION_FILE → $FS_ROOT/"
    mkdir -p "$FS_ROOT"
    cp "$MISSION_FILE" "$FS_ROOT/"
    rm -f "$FS_ROOT/mission_flow_hold_journal.txt" \
          "$FS_ROOT/mission_flow_hold_summary.json"
}
...
run_mission() {
    ...
    echo "[s173] sentai.fr per-trial dir → $FR_DIR (symlink fr_current)"
    echo "[s173] running mission_flow_hold (timeout ${MISSION_TIMEOUT_S}s)"
    echo "import mission_flow_hold; r = mission_flow_hold.run(); print('FINAL_STATUS:', r['status'], 'kp_x:', r['kp_x'], 'is_done:', r['is_done'], 'aborted:', r['aborted_by_safety'])" \
