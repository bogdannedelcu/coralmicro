# mission_s193 — OP-S10-W21-T4 phase-3 calib bringup + 7-marker pad (post-T10).
#
...
#
# WBS: OP-S10-W21-T4 phase-3.  See README.md for claim + pass criteria.
...
def _phase4_5_axis_id_rpyt(pd_state):
    """iter17 (operator-requested 2026-05-23): RPYT-based axis ID
    BEFORE handover.  Drone stays in proven-stable Classic RPYT mode
    (Z PD active) while body XY motion is induced via small pitch+roll
    pulses.  PnP captures pad-frame displacements.  Compute
    R_body_to_pad and commit_R before handover.
    Operator: "introducem detectarea axelor in RPYT, facem handover
    dupa.  Putin la stanga putin la dreapta, putin inainte putin inapoi".

    Algorithm:
      1. Capture baseline p0 (drone hovering after PD lock)
      2. +pitch pulse → drone moves body+x; brake -pitch; settle; capture p_pitch
      3. +roll  pulse → drone moves body+y; brake -roll;  settle; capture p_roll
      4. body_x_in_pad = (p_pitch - p0) / |...|
         body_y_in_pad = (p_roll  - p0) / |...|
         body_z_in_pad = body_x × body_y  (right-hand)
      5. R = column-stack(body_x_pad, body_y_pad, body_z_pad)
      6. commit_R(R)
    """
    import math
    pd_xyz, pd_thrust, pd_vz_filt = pd_state
    ...
    SETTLE_DUR_S  = 1.2
    CAPTURE_DUR_S = 0.4
    MIN_DISP_M    = 0.015     # minimum pad displacement to be considered valid

    _j("phase4_5_axis_id_rpyt", {"pulse_deg": PULSE_DEG,
                                   "pulse_dur_s": PULSE_DUR_S,
                                   "settle_dur_s": SETTLE_DUR_S})
    _fr_event("phase", "p4_5_axis_id_rpyt_start")
    _extpos_reset_gate()

    def _rpyt_z_pd_tick(roll_deg, pitch_deg):
        """One tick: send RPYT with given roll/pitch + Z PD thrust; stream
        z-only ExtPos.  Returns (n, px, py, pz) from PnP this tick."""
        nonlocal z_prev, vz_filt, last_xyz
        n, px, py, pz = _try_pnp()
        if n >= 4 and pz > 0.0:
            z_now = pz
            last_xyz = (px, py, pz)
    ...
    def _stream_rpyt(roll_deg, pitch_deg, dur_s):
        n_ticks = int(dur_s * 1000 / TICK_MS)
        for _ in range(n_ticks):
            _rpyt_z_pd_tick(roll_deg, pitch_deg)
            sentai.rtos.sleep_ms(TICK_MS)

    def _capture_pnp_median(dur_s=CAPTURE_DUR_S):
        n_ticks = int(dur_s * 1000 / TICK_MS)
        xs, ys, zs = [], [], []
        for _ in range(n_ticks):
            n, px, py, pz = _rpyt_z_pd_tick(0.0, 0.0)
            if n >= 4 and pz > 0.0:
    ...
    if p0 is None:
        _j("axis_id_rpyt", "FAIL_pnp_p0")
        _fr_event("phase", "p4_5_axis_id_FAIL_p0")
        return False

    # ── Pitch+ pulse + brake + settle → measures body+x in pad ──────────
    _stream_rpyt(0.0, +PULSE_DEG, PULSE_DUR_S)
    _stream_rpyt(0.0, -PULSE_DEG, PULSE_DUR_S)
    _stream_rpyt(0.0, 0.0, SETTLE_DUR_S)
    p_pitch = _capture_pnp_median()
    _j("axis_id_p_pitch", {"pnp": p_pitch})

    # ── Roll+ pulse + brake + settle → measures body+y in pad ───────────
    _stream_rpyt(+PULSE_DEG, 0.0, PULSE_DUR_S)
    _stream_rpyt(-PULSE_DEG, 0.0, PULSE_DUR_S)
    _stream_rpyt(0.0, 0.0, SETTLE_DUR_S)
    p_roll = _capture_pnp_median()
    _j("axis_id_p_roll", {"pnp": p_roll})

    if p_pitch is None or p_roll is None:
        _j("axis_id_rpyt", "FAIL_pnp_pulse")
        _fr_event("phase", "p4_5_axis_id_FAIL_pulse")
        return False
    ...
    _j("axis_id_vectors", {"pitch_disp": (dx_p, dy_p), "mag_p": mag_p,
                            "roll_disp":  (dx_r, dy_r), "mag_r": mag_r})

    if mag_p < MIN_DISP_M or mag_r < MIN_DISP_M:
        _j("axis_id_rpyt", "FAIL_small_displacement")
        _fr_event("phase", "p4_5_axis_id_FAIL_disp")
        return False

    # body_x and body_y in pad (NORMALIZED unit vectors).  But because
    # drone drifts during settle, these may not be exactly perpendicular.
    # Use pitch_disp as the primary body_x direction and ORTHOGONALIZE
    # roll_disp against it (Gram-Schmidt).
    bx_pad = (dx_p / mag_p, dy_p / mag_p)        # body_x in pad (XY)
    # Project roll onto body_x and subtract → orthogonal component
    ...
    if by_mag < MIN_DISP_M:
        _j("axis_id_rpyt", {"fail": "roll_too_collinear_with_pitch"})
        _fr_event("phase", "p4_5_axis_id_FAIL_collinear")
        return False
    by_pad = (by_perp[0] / by_mag, by_perp[1] / by_mag)

    # body_z in pad = body_x × body_y → since both in XY plane, z = scalar
    bz_z = bx_pad[0] * by_pad[1] - bx_pad[1] * by_pad[0]
    # Should be ±1 (right or left handed).  For camera-down mount with
    # body_z pointing UP in body but pad_z pointing UP in pad, expect
    # negative (cameras are flipped wrt body Z).  But may be +1 depending
    # on which axis we labeled +pitch.  Sign auto-detected here.

    # R_body_to_pad row-major: ROW i = i-th pad component of result
    # COL j = j-th body unit vector in pad coords
    R = (
    ...
    _fr_event("phase",
               "p4_5_axis_id_rpyt_done θ_p=%.1f° θ_r=%.1f° bz_z=%+.2f rc=%d" %
               (theta_p, theta_r, bz_z, rc))
    return True
...
def run():
    ...
    _j("mission_start", {"wbs": "OP-S10-W21-T4-phase3", "iter": 2})
    ...
    try:
        ...
        # Drone stays in proven-stable RPYT mode during axis ID.  After
        # R is committed, then handover to HL/Generic for bringup.
