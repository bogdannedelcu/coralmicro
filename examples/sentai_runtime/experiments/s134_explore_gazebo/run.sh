#!/bin/bash
# s134 — L6 sentai.explore Gazebo integration runner.
#
# Mirrors s132/run.sh: ensures SITL up, ensures gz_to_uds_bridge up,
# runs mission_explore.py (which spawns sentai_sim), then verdict.py.
...
WORKDIR=/tmp/s134_explore_gazebo
mkdir -p "$WORKDIR"
...
respawn_cf2_at_origin() {
    if [ "${S134_NO_RESPAWN:-0}" = "1" ]; then
        echo "[s134] S134_NO_RESPAWN=1 — skipping respawn (NOT reproducible!)"
        return 0
    fi
    if is_cf2_up; then
        echo "[s134] tearing down running SITL so cf2 respawns at origin"
...
ensure_sitl_up() {
    if is_cf2_up; then
        echo "[s134] cf2 SITL up on UDP 19850"
        return 0
    fi
    echo "[s134] launching SITL stack via $LAUNCH_HYBRID"
    ...
    until is_cf2_up; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s134] FAIL — SITL never came up; see $WORKDIR/sitl.log"
            return 1
        fi
        sleep 2
    done
    echo "[s134] cf2 UDP 19850 ready"
}
...
ensure_bridge_after_sim() {
    ...
    until [ -S /tmp/sentai_cam.sock ]; do
        if [ $(date +%s) -ge $deadline ]; then
            echo "[s134] FAIL — sentai_sim never opened cam.sock"
            return 1
        fi
        sleep 0.3
    done
    if bridge_alive; then
        echo "[s134] gz_to_uds_bridge already up + accepting"
        return 0
    fi
    if [ -S /tmp/sentai_flow_out.sock ]; then
        echo "[s134] stale flow_out.sock (bridge dead); removing"
        rm -f /tmp/sentai_flow_out.sock
    fi
    echo "[s134] launching gz_to_uds_bridge inside distrobox"
    ...
    until [ -S /tmp/sentai_flow_out.sock ]; do
        if [ $(date +%s) -ge $d2 ]; then
            echo "[s134] FAIL — bridge did not open flow_out.sock"
            return 1
        fi
        sleep 0.3
    done
    echo "[s134] flow_out.sock ready"
}

run_mission() {
    echo "[s134] running mission_explore.py (spawns sentai_sim + L6 round-trip)"
...
if [ "$mission_rc" -ne 0 ]; then
    echo "[s134] mission_explore.py rc=$mission_rc — see $WORKDIR/mission_host.log + repl.transcript"
fi
verdict && rc=0 || rc=$?
echo "[s134] exit code $rc"
exit $rc
