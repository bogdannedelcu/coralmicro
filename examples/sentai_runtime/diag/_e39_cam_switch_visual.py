...
def grab(tag):
    # Retry a couple of times if cam capture fails (first-post-switch
    # race).  We WANT the first successful frame, so no extra delay.
    for attempt in range(3):
        try:
            ...
            print("  %s (try %d)" % (path, attempt))
            del jpg
            gc.collect()
            return
        except RuntimeError:
            sentai.rtos.sleep_ms(25)
    print("  WARN %s: 3 fails" % tag)

# Warm both cameras — first frame after switch sometimes stale.
def _warm(cam, tries=5):
    sentai.camera.select(cam)
    for _ in range(tries):
        try:
            sentai.camera.jpeg(30)
            return True
        except RuntimeError:
            sentai.rtos.sleep_ms(50)
    return False
_warm(0); _warm(1)
_warm(0); _warm(1)  # second pass to fully stabilise
