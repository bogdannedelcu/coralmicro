...
def e19_switch_sync_check(cam_a=0, cam_b=1, repetitions=16, quality=70,
                          save=True, resolution=(512, 512)):
    """E19 — ISR-synchronised switch wake vs legacy polling drain.

    Runs TWO passes back-to-back, alternating cam0↔cam1 on every frame
    and encoding a JPEG per iteration:

      A) switch_sync(0)  — legacy vTaskDelay(1) poll loop
      B) switch_sync(1)  — CSI ISR gives semaphore when post-switch
                           countdown reaches 0; reader wakes immediately

    Both passes use the same switch_drain threshold (default whatever is
    set coming in — typically 1 so the fast path has its best chance).
    JPEGs are kept in MicroPython heap during the hot loop and written
    to LFS afterwards, so NAND latency does not leak into timing.

    Target: at resolution 512×512 on the 30 fps sensor, pass B should
    sustain ≥15 fps (≤66 ms/iter) — half of sensor FPS, i.e. one camera
    on every other frame.  Pass A is the baseline this target is
    compared against.

    Memory: ~16 reps × ~35 KB JPEG = ~560 KB per pass.  The pass-A list
    is discarded (`del` + gc.collect()) before pass B starts so the 512
    KB MP heap never holds both passes at once.

    Saves per run:
      <session>/e19_sync0_frames/NNN_camX_YYYms.jpg
      <session>/e19_sync1_frames/NNN_camX_YYYms.jpg
    and two CSVs + one summary CSV.
    """
    from diag._session import _session, begin, end

    owned_session = (_session is None)
    if owned_session:
        begin("e19_switch_sync")

    prev_verbose = sentai.verbose(0)

    # Same defensive stop as E17 — if PrepTask is alive, in-loop
    # camera.select would race with cam_grab_latest.
    if sentai.pipeline.running():
        sentai.pipeline.stop()
    ...
    try:
        ...
        # Disable the ratio auto-alternate scheduler so the only source
        # of switches is the explicit select() below.  Otherwise the
        # ISR could fire extra flips and confound the sync=0 baseline.
        ra_prev, rb_prev = sentai.camera.ratio()
        ...
        thresh = sentai.camera.switch_drain()
        print("[E19] drain=%d reps=%d quality=%d %dx%d" % (
            thresh, repetitions, quality, w, h))

        # --------------------------------------------------------------
        # One pass.  Same hot-loop shape as E17: flip, encode JPEG in RAM,
        # record wall dt.  No LFS during the loop.
        # --------------------------------------------------------------
        def _pass(sync_flag, label):
            sentai.camera.switch_sync(sync_flag)
            # Fresh warm-up under this mode so the very first measured
            # iteration is not an outlier from the mode transition.
            sentai.camera.select(cam_a); sentai.camera.to_tensor()
            sentai.camera.select(cam_b); sentai.camera.to_tensor()
            ...
            # LFS writes — cold phase, does not affect timing.
            saved_dir = None
            if save and _session is not None:
                saved_dir = "%s/e19_%s_frames" % (_session.dir, label)
                try: sentai.fs.mkdir(saved_dir)
                except Exception: pass
                for (i, cam, dt, jpg) in captures:
                    path = "%s/%03d_cam%d_%dms.jpg" % (saved_dir, i, cam, dt)
                    try: sentai.fs.write(path, jpg)
                    except Exception as e:
                        print("  WARN failed to save %s: %s" % (path, e))

            times_all = [c[2] for c in captures]
            cam_a_t = [c[2] for c in captures if c[1] == cam_a][1:]
            cam_b_t = [c[2] for c in captures if c[1] == cam_b][1:]
            sizes = [len(c[3]) for c in captures]

            st_all = stats(times_all)
            mean_ms = st_all["mean"]
            fps = 1000.0 / mean_ms if mean_ms > 0 else 0
            print("[E19:%s] hot_wall=%dms  per-frame mean=%.1fms  fps=%.2f"
                  % (label, t_run_wall, mean_ms, fps))
            if cam_a_t:
                _print_stats("  cam%d" % cam_a, stats(cam_a_t))
            if cam_b_t:
                _print_stats("  cam%d" % cam_b, stats(cam_b_t))
            print("  jpeg size: min=%d max=%d mean=%d bytes" % (
                min(sizes), max(sizes), sum(sizes) // len(sizes)))

            if save and _session is not None:
                csv_path = _save_path("e19_%s" % label)
                save_csv(csv_path,
                    ["run_index", "cam_id", "elapsed_ms", "jpeg_bytes"],
                    [(c[0], c[1], c[2], len(c[3])) for c in captures])
                _save_desc(csv_path,
                    "E19 pass %s.  Alternating cam%d<->cam%d per frame.\n"
                    "Hot loop keeps JPEGs in RAM; LFS writes after.\n"
                    % (label, cam_a, cam_b),
                    params={"cam_a": cam_a, "cam_b": cam_b,
                            "repetitions": repetitions,
                            "quality": quality,
                            "switch_drain": thresh,
                            "switch_sync": sync_flag,
                            "mean_ms": mean_ms,
                            "fps": round(fps, 2),
                            "hot_wall_ms": t_run_wall,
                            "jpeg_dir": saved_dir})
                _record("e19_%s" % label, csv_path,
                        "sync=%d mean=%.1fms fps=%.2f" % (
                            sync_flag, mean_ms, fps))
            return {"label": label, "sync": sync_flag,
                    "samples": {"elapsed_ms": times_all,
                                "cam_id": [c[1] for c in captures],
                                "jpeg_bytes": sizes},
                    "summary": {"per_frame": st_all,
        ...
        # Pass A — legacy polling
        a = _pass(0, "sync0")
        gc.collect()
        # Pass B — ISR fast-wake
        b = _pass(1, "sync1")
        gc.collect()

        # Restore a sensible default on the way out — strictly-better mode.
        sentai.camera.switch_sync(1)
        # Restore previous ratio if it was set.
        if ra_prev or rb_prev:
            sentai.camera.ratio(ra_prev, rb_prev)

        a_mean = a["summary"]["per_frame"]["mean"]
        b_mean = b["summary"]["per_frame"]["mean"]
        speedup = a_mean / b_mean if b_mean > 0 else 0
        target_fps = 15.0
        meets = b["summary"]["fps"] >= target_fps
        print("[E19] SYNC SUMMARY")
        print("  sync=0 (poll):     %.1f ms/iter, %.2f fps"
              % (a_mean, a["summary"]["fps"]))
        print("  sync=1 (ISR sem):  %.1f ms/iter, %.2f fps"
              % (b_mean, b["summary"]["fps"]))
        print("  speedup x%.2f  — target ≥%.1f fps: %s"
              % (speedup, target_fps, "OK" if meets else "MISS"))

        return {
            "experiment": "E19_switch_sync_check",
            "meta": snapshot_meta("E19_switch_sync",
                                  cam_a=cam_a, cam_b=cam_b,
                                  resolution="%dx%d" % (w, h),
                                  repetitions=repetitions,
                                  switch_drain=thresh,
                                  quality=quality),
            "passes": {"A_sync0": a, "B_sync1": b},
            "summary": {"sync0_mean_ms": a_mean,
                        "sync1_mean_ms": b_mean,
                        "sync0_fps": a["summary"]["fps"],
                        "sync1_fps": b["summary"]["fps"],
